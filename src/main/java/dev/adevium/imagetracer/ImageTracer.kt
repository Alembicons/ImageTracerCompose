...
class ImageTracer(private val image: ImageBitmap, private val options: TracingOptions) {
    private val imageData = loadImageData()
    private val palette = selectPalette()

    fun toVector(): ImageVector {
        return imageDataToVector()
    }

    private fun imageDataToVector(): ImageVector {
        val indexedImage = imageDataToTraceData()
        return VectorUtils.getVector(indexedImage, options)
    }

    // Tracing ImageData, then returning IndexedImage with tracedata in layers
    private fun imageDataToTraceData(): IndexedImage {
    ...
    private fun loadImageData(): ImageData {
    ...
    private fun selectPalette(): Array<ByteArray> {
        return if (options.algorithm == ColorQuantizationAlgorithm.Precise) {
            getPalette()
        } else {
    ...
    private fun getPalette(): Array<ByteArray> {
    ...
    private fun generatePalette(): Array<ByteArray> {
    ...
    private fun samplePalette(): Array<ByteArray> {
        val numbersOfColor = options.numberOfColors
        var idx: Int
    ...
    companion object {
        fun imageToVector(image: ImageBitmap, options: TracingOptions): ImageVector {
            val tracer = ImageTracer(image, options)
            return tracer.toVector()
        }
    }
    ...
    class TracingOptions {
        ...
        var pathomit = 8F
        // Color quantization
        var algorithm = ColorQuantizationAlgorithm.Fast
        var colorSampling = false
    ...
    enum class ColorQuantizationAlgorithm {
        Precise,
        Fast
    }
}